...
def save_pairwise_btnck_dist(dgms_path, save_path, dimension):
    ...
    save_path : string
        Path of the directory where to save the .csv file containing pairwise bottleneck distances
    dimension : int
    ...
    name_file = os.path.join(save_path, "pairwise_btnck_dist_dim{}.csv".format(dimension))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dgms', help="path containing the directory of the file with the persistence diagrams", required=True)
    parser.add_argument('--dim', help="dimension of the persistence diagrams", required=True, type=int)
    parser.add_argument('--save', help="path containing the directory where to save a csv file", required=True)
